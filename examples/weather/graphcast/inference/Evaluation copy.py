...
def main(year, month, day, hour):
    ...
    for t in range(num_steps):
        ...
        ART_pred = t_0.strftime(os.path.join(cfg.get("path",'PreddataDir'), f'%Y/%Y%m%d/CHN_PLEVELS_0P25_HOUR_%Y%m%d%H_{LT:03d}.grib2'))
        LAND_pred =  t_0.strftime(os.path.join(cfg.get("path",'PreddataDir'), f'%Y/%Y%m%d/CHN_SURFACE_0P25_HOUR_%Y%m%d%H_{LT:03d}.grib2'))
        ...
        for i in range(len(grbs_LAND_true)):
            ...
            if i==4:
                ...
                plt.savefig(f'/workspaces/AIWP_modulus/examples/weather/graphcast/inference/png/region_values_comparison_{t}.png')
